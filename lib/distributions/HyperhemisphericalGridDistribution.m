classdef HyperhemisphericalGridDistribution < AbstractHyperhemisphericalDistribution & AbstractGridDistribution
    ...
    methods
        ...
        function mu = meanDirection(this)
            warning('For hyperhemispheres, this function yields the mean and not the mode');
        ...
        function hgd = toFullSphere(this)
            ...
            assert(size(hgd.grid,2) == 2*size(this.grid,2));
        end
        ...
        function h = plotInterpolated(this, useHarmonics)
            ...
            hhgdInterp = CustomHyperhemisphericalDistribution(@(x)2*hdgd.pdf(x,useHarmonics),3);
        ...
        function sgd = fromDistribution(dist, noOfGridPoints, gridType)
            ...
            elseif isa(dist,'AbstractHypersphericalDistribution')
                warning('fromDistribution:asymmetricOnHypersphere',...
                    'Approximating a hyperspherical distribution on a hemisphere. The density does not appear to be symmetric. Double check if this is intentional.');
                fun = @(x)2*dist.pdf(x);
            end
