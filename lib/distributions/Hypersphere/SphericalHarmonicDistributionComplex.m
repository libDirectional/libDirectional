classdef SphericalHarmonicDistributionComplex < AbstractSphericalHarmonicDistribution
    ...
    methods
        function this=SphericalHarmonicDistributionComplex(coeffMat,transformation)
            % coeffMat is assumed to be a L x 2*L+1 matrix. The
            ...
            for l=0:size(coeffMat,1)-1
                assert(max(abs(coeffMat(l+1,2*l+1:-1:l+1)-(-1).^(l:-1:0).*conj(coeffMat(l+1,1:l+1))))<1e-5,...
                    'CoefficientMatrix:EntriesComplex','Entries of the coefficient matrix do not describe a real function.');
            end
            this@AbstractSphericalHarmonicDistribution(coeffMat,transformation);
        end
        ...
        function shd=toSphericalHarmonicDistributionReal(this)
            ...
            shd=SphericalHarmonicDistributionReal(coeffMat,this.transformation);
        end
        ...
        function shd=multiply(this,other,degree)
            ...
            shd=SphericalHarmonicDistributionComplex.fromPlmWithDegreeAndOrder(plm,this.transformation);
        ...
        function shd=multiplyViaCoefficients(this,other,degree)
            ...
            for l1=0:size(this.coeffMat,1)-1
                for l2=0:size(other.coeffMat,1)-1
                    ...
                    % Filter out entries for which coefficients both
            ...
            % Normalization is left up to the constructor but is expected
            warningSettings=warning('off','Normalization:notNormalized');
            shd=SphericalHarmonicDistributionComplex(coeffMat,this.transformation);
            warning(warningSettings);
        end
        function shd=rotate(this,alpha,beta,gamma)
            ...
            coeffMatRot=NaN(size(this.coeffMat));
            for l=0:size(this.coeffMat,1)-1
                for mNew=-l:l
    ...
    methods (Static)
        function shd=fromDistributionViaIntegral(dist,degree,transformation)
            ...
            shd=SphericalHarmonicDistributionComplex.fromFunctionViaIntegralCart(@(x)dist.pdf(x),degree,transformation);
        end
        function shd=fromDistributionFast(dist,degree,transformation)
            ...
            assert(isa(dist,'AbstractHypersphericalDistribution')&&dist.dim==3,'dist must be a distribution on the sphere.');
            shd=SphericalHarmonicDistributionComplex.fromFunctionFast(@dist.pdf,degree,transformation);
        end
        function shd=fromFunctionFast(fun,degree,transformation)
            ...
            shd=SphericalHarmonicDistributionComplex.fromPlmWithDegreeAndOrder(xyz2plm(fval),transformation);
        ...
        function shd=fromFunctionViaIntegralCart(fun,degree,transformation)
            ...
            % rows (x, y and z coordindates) as the input argument.
            % This is the case for all .pdf functions on the sphere.
            shd=SphericalHarmonicDistributionComplex.fromFunctionViaIntegralSph(@funSph,degree,transformation);
            function vals=funSph(theta,phi)
                theta=-theta+pi/2; % Convert to different convention for spherical coordinates
        ...
        function shd=fromFunctionViaIntegralSph(fun,degree,transformation)
            ...
            coeffMat=NaN(degree+1,2*degree+1);
            for l=0:degree
                for m=-l:l
            ...
            shd=SphericalHarmonicDistributionComplex(coeffMat,transformation);
        end
        function shd=fromPlmWithDegreeAndOrder(plm,transformation)
            coeffMat=NaN(plm(end,1)+1,2*plm(end,1)+1);
