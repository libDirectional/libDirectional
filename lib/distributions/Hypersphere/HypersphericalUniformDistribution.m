classdef HypersphericalUniformDistribution < AbstractHypersphericalDistribution
    methods
        function this=HypersphericalUniformDistribution(dim)
            this.d=dim;
        end
        function vals=pdf(this,xa)
            % pdf is always 1 divided by surface area (integrates to 1).
            vals=(1/AbstractHypersphericalDistribution.computeUnitSphereSurface(this.d))*ones(1,size(xa,2));
        end
        function X=sample(this,n)
            ...
            if this.d==3
                ...
                X=cell2mat(cellfun(@(x){x/norm(x)},num2cell(randn(this.d,n),1)));
            end
        end
    end 
end