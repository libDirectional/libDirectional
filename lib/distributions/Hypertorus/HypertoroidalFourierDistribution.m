classdef HypertoroidalFourierDistribution < AbstractHypertoroidalDistribution
    ...
    methods 
        ...
        function p=value(this,xa)
            ...
            % Alternative implementation that (unexpectededly) is not faster than
            % the one below.
        ...
        function hfd=normalize(this)
            ...
            switch this.transformation
                ...
                case 'identity'
                    indexc00=num2cell((size(this.C)+1)/2);
                    c00=real(this.C(indexc00{:}));
                    factorForId=c00*(2*pi)^(this.dim);
        ...
        function hfd=marginalizeOut(this,dimension)
            ...
            if numel(coeffs)==1 % Return a FourierDistribution if only one dimension remains
                hfd=FourierDistribution.fromComplex(reshape(2*pi*squeeze(this.C(indices{:})),[1,coeffs]),'identity');
                return
            end
            hfd.C=reshape(2*pi*squeeze(this.C(indices{:})),coeffs);
            hfd.dim=this.dim-1;
        end
    ...
    methods (Static)
        ...
        function hfd=fromDistribution(distribution,noOfCoefficients,desiredTransformation)
            ...
            if strcmp(desiredTransformation,'identity')&&isa(distribution,'HypertoroidalWNDistribution')
