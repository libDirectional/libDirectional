classdef ToroidalFourierDistribution < HypertoroidalFourierDistribution & AbstractToroidalDistribution
    ...
    methods
        ...
        function r = integral(this, l, r)
            ...
            % coefficients
            r=real(sum(sum(tfd.C.*(jVector.'*kVector))));
        end
        ...
        function p = value(this,xa) 
            ...
            % This is faster than using exp(1i*bsxfun(@plus,bsxfun(@times,(-maxk(1):maxk(1))',reshape(xa(1,:),1,1,[])),...
            % bsxfun(@times,-maxk(2):maxk(2),reshape(xa(2,:),1,1,[]))))
            p=reshape(sum(sum(bsxfun(@times,this.C,bsxfun(@times,exp(1i*bsxfun(@times,(-maxk(1):maxk(1))',reshape(xa(1,:),1,1,[]))),...
                exp(1i*bsxfun(@times,-maxk(2):maxk(2),reshape(xa(2,:),1,1,[]))))),1),2),1,[]);
    ...
    methods (Static)
        function f=fromFunction(fun,noOfCoefficients,desiredTransformation)
            ...
            if length(noOfCoefficients)==1,noOfCoefficients=noOfCoefficients*ones(1,2);end
