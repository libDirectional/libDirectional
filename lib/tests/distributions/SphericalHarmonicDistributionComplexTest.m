classdef SphericalHarmonicDistributionComplexTest< matlab.unittest.TestCase    
    methods (Test)
        function testNormalization(testCase)
            testCase.verifyWarning(@()SphericalHarmonicDistributionComplex(1),'Normalization:notNormalized');
            testCase.verifyError(@()SphericalHarmonicDistributionComplex(0),'Normalization:almostZero');
            coeffRand=rand(1,9);
            ...
            shd=SphericalHarmonicDistributionComplex(unnormalizedCoeffs);
        ...
        function testTruncation(testCase)
            ...
            unnormalizedCoeffs=[coeffRand(1),NaN,NaN,NaN,NaN;coeffRand(2)+1i*coeffRand(3),coeffRand(4),-coeffRand(2)+1i*coeffRand(3),NaN,NaN;...
                coeffRand(5)+1i*coeffRand(6),coeffRand(7)+1i*coeffRand(8),coeffRand(9),-coeffRand(7)+1i*coeffRand(8),coeffRand(5)-1i*coeffRand(6)];
            cshd=SphericalHarmonicDistributionComplex(unnormalizedCoeffs);
            warning(warningSettings);
        ...
        function testl0m0(testCase)
            shd=SphericalHarmonicDistributionComplex(1/sqrt(4*pi)); % Initialize with 1/sqrt(4*pi) and overwrite to prevent normalization
        ...
        function testl1m0(testCase)
            shd=SphericalHarmonicDistributionComplex(1/sqrt(4*pi));
        ...
        function testTransformationViaIntegral(testCase)
            ...
            if enableExpensive
                ...
                warningSettings=warning('off','Normalization:notNormalized');
                shd=SphericalHarmonicDistributionComplex.fromFunctionViaIntegralCart(@(x)dist.pdf(x),11);
                warning(warningSettings);
                points=rand(3,10000);
                testCase.verifyEqual(shd.pdf(points),dist.pdf(points),'AbsTol',2E-3);
                
                % Test approximating a spherical harmonic distrubution
                dist=SphericalHarmonicDistributionComplex([sqrt(1/pi)/2,NaN,NaN,NaN,NaN,NaN,NaN;0,0,0,NaN,NaN,NaN,NaN;0,0,0,0,0,NaN,NaN;1/sqrt(2),0,0,0,0,0,-1/sqrt(2)]);
                warningSettings=warning('off','Normalization:notNormalized');
                shd=SphericalHarmonicDistributionComplex.fromFunctionViaIntegralCart(@(x)dist.pdf(x),3);
                warning(warningSettings);
        ...
        function testCovergence(testCase)
            ...
            diffs=NaN(noDiffs,1);
            for i=2:noDiffs+1
                shd=SphericalHarmonicDistributionComplex.fromFunctionViaIntegralCart(@(x)dist.pdf(x),i);
        ...
        function testMultiplicationFixedCoeffs(testCase)
            ...
            cshd1=SphericalHarmonicDistributionComplex([sqrt(1/pi)/2,NaN,NaN;0,1,0]);
            cshd2=SphericalHarmonicDistributionComplex([sqrt(1/pi)/2,NaN,NaN,NaN,NaN,NaN,NaN;0,0,0,NaN,NaN,NaN,NaN;0,0,0,0,0,NaN,NaN;1i/sqrt(2),0,0,0,0,0,1i/sqrt(2)]);
            ...
            cshd1=SphericalHarmonicDistributionComplex([sqrt(1/pi)/2,NaN,NaN;sqrt(1/2),0,-sqrt(1/2)]);
            cshd2=SphericalHarmonicDistributionComplex([sqrt(1/pi)/2,NaN,NaN,NaN,NaN,NaN,NaN;0,0,0,NaN,NaN,NaN,NaN;0,0,0,0,0,NaN,NaN;1i/sqrt(2),0,0,0,0,0,1i/sqrt(2)]);
            cshdMult=cshd1.multiply(cshd2,size(cshd1.coeffMat,1)+size(cshd2.coeffMat,1)-2);
            testCase.verifyEqual(diff(cshdMult.pdf([x;y;z])./(cshd1.pdf([x;y;z]).*cshd2.pdf([x;y;z]))),zeros(1,size(x,2)-1),'AbsTol',1E-6);
            
            cshd1=SphericalHarmonicDistributionComplex([sqrt(1/pi)/2,NaN,NaN,NaN,NaN;1i*sqrt(1/2),0,1i*sqrt(1/2),NaN,NaN;0,0,0,0,0]);
            cshd2=SphericalHarmonicDistributionComplex([sqrt(1/pi)/2,NaN,NaN,NaN,NaN;0,1,0,NaN,NaN;0,0,0,0,0]);
            cshdMult=cshd1.multiply(cshd2,size(cshd1.coeffMat,1)+size(cshd2.coeffMat,1)-2);
            testCase.verifyEqual(diff(cshdMult.pdf([x;y;z])./(cshd1.pdf([x;y;z]).*cshd2.pdf([x;y;z]))),zeros(1,size(x,2)-1),'AbsTol',1E-6);
            
            cshd1=SphericalHarmonicDistributionComplex([sqrt(1/pi)/2,NaN,NaN,NaN,NaN,NaN,NaN;0,0,0,NaN,NaN,NaN,NaN;0,0,0,0,0,NaN,NaN;1i/sqrt(2),0,0,0,0,0,1i/sqrt(2)]);
            cshd2=SphericalHarmonicDistributionComplex([sqrt(1/pi)/2,NaN,NaN,NaN,NaN,NaN,NaN;0,0,0,NaN,NaN,NaN,NaN;0,0,0,0,0,NaN,NaN;0,0,0,1,0,0,0]);
            cshdMult=cshd1.multiply(cshd2,size(cshd1.coeffMat,1)+size(cshd2.coeffMat,1)-2);
            testCase.verifyEqual(diff(cshdMult.pdf([x;y;z])./(cshd1.pdf([x;y;z]).*cshd2.pdf([x;y;z]))),zeros(1,size(x,2)-1),'AbsTol',1E-6);
        end
        function testMultiplicationVsVmf(testCase)
            vmf=VMFDistribution([0;-1;0],0.5);
            shd=SphericalHarmonicDistributionComplex.fromFunctionViaIntegralCart(@(x)vmf.pdf(x),15);
            vmfCurr=vmf;
            shdCurr=shd;
            for i=1:5
        ...
        function testMultiplicationViaTransform(testCase)
            ...
            for transformation={'identity','sqrt'}
                shd1=SphericalHarmonicDistributionComplex.fromDistributionFast(VMFDistribution([1;1;1]/sqrt(3),2),5,[transformation{:}]);
                shd2=SphericalHarmonicDistributionComplex.fromDistributionFast(VMFDistribution([1;1;-1]/sqrt(3),2),5,[transformation{:}]);
        ...
        function testRotationGrid(testCase)
            ...
            for degree=7:10
                for i=1:numel(alphas)
                    ...
                    shd=rshd.toSphericalHarmonicDistributionComplex;
        ...
        function testMean(testCase)
            shd=SphericalHarmonicDistributionComplex([1/sqrt(4*pi),NaN,NaN;0,1,0]);
            testCase.verifyEqual(shd.mean,[0;0;1],'AbsTol',1E-6);
            shd=SphericalHarmonicDistributionComplex([1/sqrt(4*pi),NaN,NaN;0,-1,0]);
            testCase.verifyEqual(shd.mean,[0;0;-1],'AbsTol',1E-6);
            shd=SphericalHarmonicDistributionComplex([1/sqrt(4*pi),NaN,NaN;1i*sqrt(1/2),0,1i*sqrt(1/2)]);
            testCase.verifyEqual(shd.mean,[0;1;0],'AbsTol',1E-6);
            shd=SphericalHarmonicDistributionComplex([1/sqrt(4*pi),NaN,NaN;-1i*sqrt(1/2),0,-1i*sqrt(1/2)]);
            testCase.verifyEqual(shd.mean,[0;-1;0],'AbsTol',1E-6);
            shd=SphericalHarmonicDistributionComplex([1/sqrt(4*pi),NaN,NaN;sqrt(1/2),0,-sqrt(1/2)]);
            testCase.verifyEqual(shd.mean,[1;0;0],'AbsTol',1E-6);
            shd=SphericalHarmonicDistributionComplex([1/sqrt(4*pi),NaN,NaN;-sqrt(1/2),0,sqrt(1/2)]);
            testCase.verifyEqual(shd.mean,[-1;0;0],'AbsTol',1E-6);
            shd=SphericalHarmonicDistributionComplex([1/sqrt(4*pi),NaN,NaN;-1i*sqrt(1/2)-sqrt(1/2),0,-1i*sqrt(1/2)+sqrt(1/2)]);
            testCase.verifyEqual(shd.mean,1/sqrt(2)*[-1;-1;0],'AbsTol',1E-6);
            shd=SphericalHarmonicDistributionComplex([1/sqrt(4*pi),NaN,NaN;1i*sqrt(1/2)+sqrt(1/2),1,1i*sqrt(1/2)-sqrt(1/2)]);
        ...
        function testFromDistributionFast(testCase)
            ...
            for transformation={'identity','sqrt'}
                for i=1:numel(dists)
                    shd=SphericalHarmonicDistributionComplex.fromDistributionFast(dists{i},21,[transformation{:}]);
    ...
    methods
        function testConversion(testCase,coeffMat)
            warningSettings=warning('off','Normalization:notNormalized');
            cshd=SphericalHarmonicDistributionComplex(coeffMat);
            warning(warningSettings);
            rshd=cshd.toSphericalHarmonicDistributionReal;
            [phi,theta]=meshgrid(linspace(0,2*pi,10),linspace(-pi/2,pi/2,10));
            [x,y,z]=sph2cart(phi(:)',theta(:)',1);
