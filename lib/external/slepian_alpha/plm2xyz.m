...
% Last modified by Florian Pfaff for libDirectional, 14/04/2016
...
%    disp(sprintf('Creating %i by %i grid with resolution %8.3f',nlat,nlon,degres))
  ...
  for ldeg=1:size(els,1)
    ...
    % First case ensured that it is only loaded from file once (massively
    % improves speed if called multiple times for same nlat and lon.
    if (size(legendreCell,1)>lup && size(legendreCell,2)>nlat && ~isempty(legendreCell{lup+1,nlat+1}) &&ldown==0 ... 
            && length(c11cmn)==4 && all(c11cmn==[0 90 360 -90])...
            && ~(size(els,1)==1 &&  nargin==6))
      Plm=legendreCell{lup+1,nlat+1};
  
    else
        mfn=mfilename('fullpath');
        fnpl=fullfile(mfn(1:end-8),'LEGENDRE',sprintf('LSSM-%i-%i-%i.mat',ldown,lup,nlat));
