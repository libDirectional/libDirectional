function [s_cap,n_regions] = eq_caps_symm(dim,N)
%EQ_CAPS_SYMM Partition a sphere into to nested spherical caps
%
%Syntax
% [s_cap,n_regions] = eq_caps_symm(N);
%
%Description
% [S_CAP,N_REGIONS] = EQ_CAPS_symm(N) does the following:
...
%
%Examples
% > [s_cap,n_regions] = eq_caps_symm(2,10)
% s_cap =
%     0.6435    1.5708    2.4981    3.1416
% n_regions =
%      1     4     4     1
%  
% > [s_cap,n_regions] = eq_caps_symm(3,6)
% s_cap =
...
elseif N == 1
    ...
    n_collars(enough) = 2 * max(1/2,round(0.5*(pi-2*c_polar(enough))./a_ideal(enough)));
    %
