function points_s = eq_point_set_polar_symm(dim, N, deleteHalf, varargin)
...
if ~dim==3
	warning('Dim is not 3. If you want to use this function for quaternions, this the wrong dimension was chosen.');
end
...
%
[a_cap, n_regions] = eq_caps_symm(dim,N);
%
    ...
    for collar_n = 1:n_collars
        ...
        if use_cache
            ...
            if twin_collar_n <= cache_size && ...
            	...
            	% Do not call symmetric version when recursing into lower dimensions
...
% end function
if deleteHalf
    points_s(:,size(points_s,2)/2+1:end)=[];
else % Symmetrize
    points_s(:,size(points_s,2)/2+1:end)=[];
    points_s=[points_s,[zeros(dim-1,1);pi]+points_s(:,2:end).*[ones(dim-1,1);-1],[zeros(dim-1,1);pi]];
end