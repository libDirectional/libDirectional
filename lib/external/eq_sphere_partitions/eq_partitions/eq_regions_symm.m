function [regions,dim_1_rot] = eq_regions_symm(dim, N, deleteHalf, symmetryType, varargin)
...
%
[s_cap, n_regions] = eq_caps_symm(dim,N);
%
...
if deleteHalf
    ...
    switch symmetryType
        case 'asymm' % Allow asymmetry
        case 'mirror' % Already symmetric
            regions(:,:,N/2+1:end)=[];
            reg=regions;
            reg(dim,[1,2],:)=pi-reg(dim,[2,1],:);
            reg(1,:,:)=reg(1,:,:)+pi;
            regions=cat(3,regions,reg);
        case 'plane'
            regions(:,:,N/2+1:end)=[];
            reg=regions;
            reg(dim,[1,2],:)=pi-reg(dim,[2,1],:);
            regions=cat(3,regions,reg);
        otherwise
            error('Unknown type of symmetry chosen');
    end
end